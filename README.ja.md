...
長時間実行、ワークフローのオーケストレーション、再開可能なハンドオフ、実行証明のための Codex カスタムスキル集です。
...
Codex の作業を単発の編集ではなく、信頼できる実行ハーネスとして扱うスキルです。

次のような場面に向いています。

- コンテキスト収集、実装、検証がすべて必要な非自明なコーディング作業
- "look into this" や "best way" のように、柔らかい表現でも実際の行動を求める依頼
- tool discipline、delegation、persistent state、recovery を組み合わせる必要がある作業
- テスト、ビルド、diagnostics、artifact、または明確な blocker によって完了を証明する必要がある作業

特に重視する点:

- 実行前の intent classification
- ファイル、コマンド、ドキュメント、artifact に基づく具体的なコンテキスト
- local、background、persistent、delegated、dependency-gated lane への作業ルーティング
- 狭い検証から広い検証へ進む verification ladder
- 推測ではなく戦略変更による failure recovery
...
Codex の作業が本当に開始され、途中で生き続け、phase を進み、最終状態に到達したことをファイルベースで残すスキルです。

次のような場面に向いています。

- チャット出力だけでは信頼が足りない作業
- 外部から完了を検証したい長いワークフロー
- 機械可読な証明と人間向けの証明を両方残したい場合
- 1つの論理 run の下で複数の attempt が発生しうる作業

特に重視する点:

- `run_id` と `attempt_id` の分離
- heartbeat による継続実行の証明
- `.codex/runs/...` 配下の machine proof
- すぐ確認できる `proof.md`
...
- 非自明なエンジニアリング作業の上位実行ループには `ddukddak`
- 全体のワークフローを構造化するには `orchestrator`
- その中の一部が長時間実行プロセスになる場合は `runner`
- そのワークフローやプロセスを後で再開する必要がある場合は `continuity-handoff`
- 本当に最後まで動いたことを外部から確認したい場合は `execution-proof`

一般的な流れ:

1. `ddukddak` で intent を分類し、コンテキストを集め、ツールを選び、実行・検証・復旧します。
2. 作業が段階、blocker、並列 workstream に分かれる場合は `orchestrator` で構造化します。
3. サーバー、ビルド、長時間実行コマンドは `runner` で追跡します。
4. 停止、要約、後日の再開が必要なときは `continuity-handoff` で正確な状態を残します。
5. 実際の進行と完了を検証可能にしたいときは `execution-proof` で proof artifact を残します。
